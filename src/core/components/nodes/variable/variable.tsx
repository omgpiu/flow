...
export const VariableNode = memo(({ id }: Props) => {
  ...
  return (
    <Container>
      ...
      <Header
        onDelete={onDelete}
        title={NODES_NAME[Nodes.VARIABLE_NODE]}
        onSave={onSave}
      >
        <div className={style.innerModal}>
          ...
          <select
            ...
            defaultValue={scope}
          >
            {SCOPE_OPTIONS.map(({ value, label }, idx) => (
          ...
          {scope == SCOPE_OPTIONS[1].value ? (
            <div>
              ...
              <select
                ...
                defaultValue={scope}
              >
                {VARIABLE_TYPE_OPTIONS.map(({ value, label }, idx) => (
                  <option key={idx} value={value}>
                    {label}
                  </option>
                ))}
                ...
                return (
                  <Fragment key={idx}>
                    <input
                      type="text"
                      name="param"
                      onChange={(e) => setAddVariables(e, idx)}
                    />
                    <input
                      type="text"
                      name="value"
