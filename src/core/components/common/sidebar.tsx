...
export const SideBar = memo(
  ({ addNode, generateNodes, serializeNodes }: Props) => {
    ...
    return (
      <aside>
        ...
        <div
          ...
          onDragStart={(event) => onDragStart(event, Nodes.WORK_FLOW)}
          draggable
        >
          {NODES_NAME[Nodes.WORK_FLOW]}
        </div>
        <div
          className="dndnode question"
          onDragStart={(event) => onDragStart(event, Nodes.PLACEHOLDER)}
          draggable
        >
          {NODES_NAME[Nodes.PLACEHOLDER]}
        </div>
        ...
        <button style={{ marginTop: "10px" }} onClick={serializeNodes}>
          SerializeNodes
        </button>
        <Login />
      </aside>
