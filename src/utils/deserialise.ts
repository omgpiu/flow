...
let count = 0;
const getDoubledIdx = (idx: string) =>
  isNaN(Number(idx)) ? String(parseInt(idx)) : idx;
  ...
  if (!Array.isArray(tree)) {
    if (!tree.skip) {
      if (tree.type !== undefined) {
        count++;
        const { type, pos_x, pos_y, _node, source, Blocks, target, ...rest } =
          tree;
        if (target) {
...
export const desirialiseAPINode = (
  data: any,
  IDX?: number,
  reqNumber?: number
) => {
  ...
  recursiveCreateNodes({
    tree: updatedNodesTree,
    nodes: accNode,
    edges: accEdge,
    cache: targetCache,
  });
